...
vec3 heatmap(const float val) {
    ...
    const vec3 hsv = vec3(hue + clamp(val, 0.f, 1.f) * -hue, 1, 1); // from blue to red
    // map hsv to rgb
...
void main() {
    ...
    out_col = vec4(abs(col - eve) / max(1e-6, sqrt(luma(col))), 1);
    out_col.rgb = heatmap(5 * luma(out_col.rgb));
}
